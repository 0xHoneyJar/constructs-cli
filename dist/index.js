...
// src/index.ts
var REGISTRY_URL = "https://raw.githubusercontent.com/0xHoneyJar/constructs-cli/main/registry.yaml";
var CONSTRUCTS_DIR = ".claude/constructs/packs";
var COMMANDS_DIR = ".claude/commands";
var VERSION = "0.1.0";
...
function symlinkCommands(slug, packDir) {
...
function unlinkCommands(slug, packDir) {
  const commandsSource = join(packDir, "commands");
  if (!existsSync(commandsSource))
    return;
...
async function info(slug) {
  ...
  if (construct.description)
    console.log(`  ${construct.description}`);
  if (construct.category)
    console.log(`  Category: ${cyan(construct.category)}`);
  if (construct.author)
    console.log(`  Author:   ${construct.author}`);
